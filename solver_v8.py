...
def run_solver():
    ...
    for day in range(1, 8):
        ...
        # Use a simple percentile for the threshold
        outlier_threshold = df_day['total_receipts_amount'].quantile(0.95)
        print(f"  Using outlier threshold: ${outlier_threshold:.2f}")

        df_outliers = df_day[df_day['total_receipts_amount'] > outlier_threshold]
        if df_outliers.empty:
            print("  No outliers found for this day.")
            continue

        best_day_mae = float('inf')
        best_params = {}

        rate_search = [x / 100 for x in range(0, 51, 1)]
        penalty_search = np.arange(-500, 501, 50)

        for rate in rate_search:
            for penalty in penalty_search:
                params = {day: {'outlier_threshold': outlier_threshold, 'outlier_rate': rate, 'outlier_penalty': penalty}}
        ...
        print(f"  Best MAE for day {day} outliers: ${best_day_mae:.2f}")
