...
def run_solver():
    ...
    for day in range(1, 8):
        ...
        threshold_search = np.arange(500, 2501, 100)
        rate_search = [x / 100 for x in range(0, 51, 2)]
        penalty_search = np.arange(-500, 501, 100)

        for threshold in threshold_search:
            df_outliers = df_day[df_day['total_receipts_amount'] > threshold]
            if df_outliers.empty:
                continue
            
            # Find the best rate and penalty for this threshold
            best_mae_for_threshold = float('inf')
            best_rate_for_threshold = 0
            best_penalty_for_threshold = 0

            for rate in rate_search:
                for penalty in penalty_search:
                    params = {day: {'outlier_threshold': threshold, 'outlier_rate': rate, 'outlier_penalty': penalty}}
                    calc = ReimbursementCalculator(rules=current_rules, params=params)
                    predicted = df_outliers.apply(lambda r: calc.calculate(r.trip_duration_days, r.miles_traveled, r.total_receipts_amount), axis=1)
                    mae = mean_absolute_error(df_outliers['expected_output'], predicted)

                    if mae < best_mae_for_threshold:
                        best_mae_for_threshold = mae
                        best_rate_for_threshold = rate
                        best_penalty_for_threshold = penalty
            
            # Now calculate the MAE for the entire day using these best parameters
            params = {day: {'outlier_threshold': threshold, 'outlier_rate': best_rate_for_threshold, 'outlier_penalty': best_penalty_for_threshold}}
            calc = ReimbursementCalculator(rules=current_rules, params=params)
            ...
            if total_mae < best_day_mae:
                ...
                best_params = params[day]
                print(f"  New best MAE for day {day}: ${total_mae:.2f} with params: {best_params}")

        final_params[day] = best_params
